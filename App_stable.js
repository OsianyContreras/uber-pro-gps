      ...
      */
      ...
      const handleEditToggle = () => {
        if (isEditing) {
          ...
          if (textToSave) {
            // Si parece coordenada (contiene numeros y coma)
            if (textToSave.includes(',') && /[0-9]/.test(textToSave)) {
              setCoordinates(textToSave);
            } else {
              setAddress(textToSave);
            }
            setExtractedLocation(textToSave);
          }
        } else {
          // Entrar en modo edición
          setEditableText(coordinates || address || extractedLocation);
        }
        setIsEditing(!isEditing);
      };
      const clearHistory = () => {
        setHistory([]);
        localStorage.removeItem('ubertronHistory');
      };
      const restoreHistoryItem = item => {
        ...
        setText(item.text || '');
        setExtractedLocation(item.text || '');

        // Intentar re-extraer info si el texto es largo
        if (item.text && item.text.length > 5) {
          extractLocationInfo(item.text);
        }
        setActiveTab('results');
      };
